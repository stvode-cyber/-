...
process.chdir(join(__d, 'frontend'));
// 将 node.exe 所在目录加入 PATH，这样可以直接调用 npm.cmd（文件名无空格）
const nodeDir = dirname(process.execPath);
const sep = process.platform === 'win32' ? ';' : ':';
process.env.PATH = nodeDir + sep + (process.env.PATH || '');
console.log('[INSTALL-FE] cwd=' + process.cwd());
console.log('[INSTALL-FE] node=' + process.execPath);
console.log('[INSTALL-FE] nodeDir added to PATH: ' + nodeDir);
// npm.cmd 文件名无空格，直接调用，shell:true 使用 cmd.exe
...
cp.on('close', c => { console.log('\n[INSTALL-FE] exit_code=' + c); process.exit(c || 0); });
